...
export default function SavedPage() {
  ...
  const groupedServices = ['Hair', 'Makeup', 'Lashes'].reduce<Record<string, Service[]>>((acc, cat) => {
  ...
  return (
    <main style={{ fontFamily: jost, background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 16px 60px' }}>
        {isLoading && (<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(255px, 1fr))', gap: 12, marginTop: 8 }}>{Array.from({ length: 6 }).map((_, i) => (<div key={i} style={{ background: 'var(--bg-card)', borderRadius: 14, height: 120, opacity: 0.3, border: '1px solid var(--border)' }} />))}</div>)}
        ...
        {!isLoading && user && totalSaved > 0 && (
          <div>
            ...
            {savedVendors.length > 0 && (
              <div style={{ marginBottom: 40 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                  <h2 style={{ fontFamily: play, fontSize: 18, fontWeight: 700, color: 'var(--text)', margin: 0 }}>Bridal and Events</h2>
                  <span style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: jost }}>{savedVendors.length} saved</span>
                </div>
            ...
            {savedServices.length > 0 && (
              <div style={{ marginBottom: 40 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                  <h2 style={{ fontFamily: play, fontSize: 18, fontWeight: 700, color: 'var(--text)', margin: 0 }}>Services</h2>
                  <span style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: jost }}>{savedServices.length} saved</span>
                </div>
