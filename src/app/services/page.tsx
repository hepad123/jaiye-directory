    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', fontFamily: 'var(--font-jost, sans-serif)' }}>
        ...
        <div style={{ background: 'var(--text-dark, #1C1917)', color: '#fff', padding: '56px 24px 48px', textAlign: 'center' }}>
          <p style={{ fontFamily: 'var(--font-playfair, serif)', fontSize: 13, letterSpacing: 3, textTransform: 'uppercase', color: 'var(--accent)', marginBottom: 12 }}>
            Beauty &amp; Personal Care
          </p>
          <h1 style={{ fontFamily: 'var(--font-playfair, serif)', fontSize: 'clamp(32px, 5vw, 52px)', fontWeight: 700, lineHeight: 1.15, margin: '0 0 16px' }}>
            Find Your Glam
          </h1>
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', display: 'flex' }}>
            {Object.keys(CATEGORIES).map(cat => (
              <button key={cat} onClick={() => setActiveCategory(cat)} style={{
                ...
                fontFamily: 'var(--font-jost, sans-serif)', fontSize: 15,
...
function ServiceCard({ service, isSaved, isSaving, onToggleSave, isLoggedIn }: {
  service: Service
  isSaved: boolean
  isSaving: boolean
  onToggleSave: () => void
  ...
  const instagramUrl = service.instagram ? 'https://instagram.com/' + service.instagram : null
  const phoneUrl     = service.phone     ? 'tel:' + service.phone : null
  const locationText = [service.location, service.city].filter(Boolean).join(', ')
  const priceText    = service.price_from ? '\u20a6' + service.price_from.toLocaleString() : 'Price on request'

  return (
    <div
      style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', transition: 'transform 0.15s ease, box-shadow 0.15s ease' }}
    ...
    >
      ...
      <div style={{ padding: 20 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
              <h3 style={{ fontFamily: 'var(--font-playfair, serif)', fontSize: 16, fontWeight: 700, color: 'var(--text)', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {service.name}
              </h3>
              {service.verified && <span style={{ fontSize: 13, flexShrink: 0, color: 'var(--accent)' }}>✓</span>}
            </div>
            <span style={{ display: 'inline-block', padding: '2px 10px', borderRadius: 999, background: accentColor + '18', color: accentColor, fontSize: 11, fontWeight: 600 }}>
              {service.subcategory}
            </span>
          </div>
          <button
            ...
            style={{ background: isSaved ? 'var(--accent)' : 'transparent', border: '1px solid', borderColor: isSaved ? 'var(--accent)' : 'var(--border)', borderRadius: 8, width: 34, height: 34, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: isLoggedIn ? 'pointer' : 'default', opacity: isSaving ? 0.5 : 1, transition: 'all 0.15s', flexShrink: 0, marginLeft: 8 }}
          >
        ...
        {service.bio && (
          <p style={{ fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.5, margin: '0 0 14px', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
            {service.bio}
          </p>
        )}
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {instagramUrl && (
              ...
              style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: 8, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 12, color: 'var(--text-muted)', textDecoration: 'none', fontFamily: 'var(--font-jost, sans-serif)', transition: 'all 0.15s' }}
